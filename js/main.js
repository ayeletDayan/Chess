...
function getAllPossibleCoordsRook(pieceCoord) {
    ...
    for (var k = 1; k < 8 - pieceCoord.i; k++) {
        var nextCoord = { i: pieceCoord.i + k, j: pieceCoord.j }
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
        else break
    }
    for (var k = 1; k <= pieceCoord.i; k++) {
        var nextCoord = { i: pieceCoord.i - k, j: pieceCoord.j }
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
        else break
    }
    for (var k = 1; k < 8 - pieceCoord.j; k++) {
        nextCoord = { i: pieceCoord.i, j: pieceCoord.j + k }
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
        else break

    }
    for (var k = 1; k <= pieceCoord.j; k++) {
        nextCoord = { i: pieceCoord.i, j: pieceCoord.j - k }
        if (isEmptyCell(nextCoord)) res.push(nextCoord);
        else break
    }
...
function getAllPossibleCoordsBishop(pieceCoord) {
    ...
    i = pieceCoord.i + 1;//(2,4)
    for (var idx = pieceCoord.j + 1; i >= 0 && i < 8; idx++) {
        console.log(i, idx)
        var coord = { i: i++, j: idx };
        if (!isEmptyCell(coord)) break;
        console.log(coord)
...
function checkCoord(i, j) {
    var coord = {};
    if (i < 8 && j < 8 && i >= 0 && j >= 0) coord = { i: i, j: j }
    return coord
    console.log(coord)
}
...
function getAllPossibleCoordsKnight(pieceCoord) {
    var res = []
    var idxI = pieceCoord.i + 1
    var idxJ = pieceCoord.j + 2
    if (isEmptyCell(checkCoord(idxI, idxJ))) res.push(checkCoord(idxI, idxJ))
    // idxI = pieceCoord.i + 1
    // idxJ = pieceCoord.j - 2
    // if (isEmptyCell(coord)) res.push(checkCoord(idxI, idxJ))
    // idxI = pieceCoord.i - 1
    // idxJ = pieceCoord.j - 2
    // if (isEmptyCell(coord)) res.push(checkCoord(idxI, idxJ))
    // idxI = pieceCoord.i - 1
